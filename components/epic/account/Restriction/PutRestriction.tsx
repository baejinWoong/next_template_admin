...
import { getRestrictionMessages, postMemberRestrictionInfo } from 'api/epicApi'
 ...
 *
 */
const PutRestriction = (props: I_props) => {
  const { userInfo, searchRestrictionLogs } = props
  ...
  const clickRestrictionAddHandler = () => {
    if (userInfo) {
      setConfirmState({
        isOpen: true,
        alertText: '이용제한을 설정하시겠습니까?',
        completeButtonCallback: () => {
          void postMemberRestrictionInfo({
  ...
  const clickResetHandler = () => {
    setConfirmState({
      isOpen: true,
      alertText: '지금까지 작성한 내용이 모두 초기화 됩니다.\n 정말 초기화 하시겠습니까?',
      completeButtonCallback: () => {
      ...
      <Card style={{ padding: '12px 16px' }}>
        <Grid type="container">
          ...
          <Grid type="item" area={3}>
            <Button type={isActive ? 'secondary' : 'primary'} onClick={changeRestrictionCompoActive}>
              {isActive ? '등록 취소' : '등록 하기'}
            </Button>
          </Grid>
        </Grid>
      </Card>
