...
import { getMemberExcelDownload, getNormallyMembers, putMemberAccountType } from 'api/epicApi'
...
const DEFAULTSTATUS: Array<'N' | 'C'> = ['N']
...
const Index: NextPage<{ page: number; size: number }> = (props: { page: number; size: number }) => {
  ...
  const searchUserHandler = () => {
    ...
    setChangeAccountTypeLists([])
    void getNormallyMembers({ keyword: searchValue, page: 1, pageSize: tableState.rowsPerPage, type: DEFAULTSTATUS, state: searchStatusSelectList }).then(
      (response) => {
        if (response.data?.status.code === 'E20002') {
          setAccounts([])
          setTableState({ page: 1, rowsPerPage: tableState.rowsPerPage, totalCount: 0 })
  ...
  const clickExcelDownloadHandler = () => {
    ...
    void getToken().then(() => {
      void getMemberExcelDownload(params).then((response) => {
        ...
        downloadTag.download = `${moment().format('YYYY_MM_DD')}_EPIC일반회원목록.csv`
        ...
        <Grid type="container" horizen="center">
          ...
          <Grid type="item" area={8} style={{ padding: '0' }}>
            <Grid type="container" align="right">
              <Grid type="item" area={8}>
                <Button onClick={() => changeAccountTypeHandler('C')} color="red">
                  회사계정으로 변경
                </Button>
              </Grid>
            </Grid>
