...
import { postMember } from 'api/adminApi'
import { I_detailMenuSet, detailMenuLists, headerMenuLists } from 'components/common/Navigation/menuLists'
import { getMenuFlat, getMenusAll } from 'components/common/Navigation/util'
...
const AdminAccountAdd = () => {
  ...
  const [targetUserInfo] = React.useState<I_userInfo>(defaultUserInfo)
  ...
  const insertUserHandler = () => {
    if (!checkedRegType(changeUserInfo.email, 'email')) {
      setAlertModalState({
        alertText: '이메일 형식이 올바르지 않습니다.',
        isOpen: true,
      })
    } else if (isCheckedInsertUserInfo()) {
      setConfirmModalState({
        alertText: '추가 하시겠습니까?',
        isOpen: true,
        completeButtonCallback: () => {
          void postMember(changeUserInfo).then((response) => {
            if (response.data?.status.code === 'E20001') {
              setAlertModalState({
                ...alertModalState,
                alertText: '추가 되었습니다.',
                isOpen: true,
  ...
  React.useEffect(() => {
    ...
    setChangeUserInfo((prev) => ({ ...prev, department: userInfoData.department }))
  }, [])
  ...
  return (
    <Grid type="container">
      ...
      <Grid type="item">
        <Grid type="container" style={{ columnGap: '12.5%', rowGap: '16px', marginTop: '24px' }}>
          <Grid type="item" area={6}>
            <Typography fontSize="element1" colorWeight="500">
              아이디
            </Typography>
            <Input
              placeholder="xx.xxx@elwide.com"
              id="userEmail"
              value={changeUserInfo?.email || ''}
              onChange={(e) => {
                changeUserEmailHandler(e.target.value)
              }}
            />
          </Grid>
          <Grid type="item" area={6}>
          ...
          <Grid type="item" area={6}>
            ...
            <Dropdown onChange={changeUserDepartmentHandler} value={changeUserInfo?.department || ''} readOnly={userInfo?.role !== 'ROLE_MASTER'}>
          ...
          <Grid type="item" area={6}>
            ...
            <Dropdown onChange={changeUserPositionHandler} value={changeUserInfo?.position || ''}>
          ...
          <Grid type="item" area={6}>
            ...
            <Dropdown onChange={changeUserStatusHandler} value={changeUserInfo.status}>
          ...
          <Grid type="item" area={6}>
            ...
            <Dropdown onChange={changeUserRoleHandler} value={changeUserInfo?.roleCode || ''}>
              {checkUserRoleValue(userInfo?.role ?? 'ROLE_USER') < 1 && <Option value="ROLE_MASTER">Master</Option>}
              {checkUserRoleValue(userInfo?.role ?? 'ROLE_USER') < 2 && <Option value="ROLE_ADMIN">Admin</Option>}
      ...
      <Grid type="item" style={{ marginTop: '60px' }}>
        <Grid type="container" align="center" style={{ columnGap: '2%' }}>
          ...
          <Grid type="item" area={3}>
            <Button type={isChangeUserInfo || isChangeUserMenuAuth ? 'primary' : 'disabled'} onClick={insertUserHandler}>
              등록
            </Button>
          </Grid>
        </Grid>
