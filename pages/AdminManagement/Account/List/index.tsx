...
import { getActiveMembers } from 'api/adminApi'
...
const Index: NextPage<{ page: number; size: number }> = (props: { page: number; size: number }) => {
  ...
  const [pageAuth, setPageAuth] = React.useState({
    menuRead: false,
    menuCreate: false,
    menuDelete: false,
  })
  ...
  const searchAdminHandler = () => {
    ...
    void getActiveMembers({ keyword: searchValue, page: 1, pageSize: tableState.rowsPerPage, type: searchSelect, status: 'y' }).then((response) => {
  ...
  const addUserHandler = () => {
    void router.push(`${router.pathname}/AdminAccountAdd`)
  }
  ...
  const changeTableRowPerPageHandler = (size: number) => {
    void getActiveMembers({ keyword: searchValue, page: 1, pageSize: size, type: searchSelect, status: 'y' }).then((response) => {
      if (response.data?.status.code === 'E20000') {
        setMembers(response.data.data.content)
        setTableState((prevState) => {
          return {
      ...
      <Grid type="item">
        <Card>
          <Grid type="container">
            ...
            <Grid type="item" area={5.5}>
              <Grid type="container" align="right">
                {pageAuth.menuCreate && (
                  <Button type="secondary" widthSize="middum" color="purple" onClick={addUserHandler}>
                    관리자 등록
                  </Button>
                )}
              </Grid>
                ...
                {members.length > 0 ? (
                  members.map((row, index) => (
                    ...
                    >
                      ...
                      <TableCell align="center">
                        <Typography fontSize="element1" color={convertStatus[row.status].rowColor} colorWeight={convertStatus[row.status].rowColorWeight}>
                          {row.lastLoginTime ? moment(row.lastLoginTime).format('YYYY-MM-DD HH:mm:ss') : '-'}
                        </Typography>
                      </TableCell>
                      <TableCell align="center">
                        <Typography fontSize="element1" color={convertStatus[row.status].rowColor} colorWeight={convertStatus[row.status].rowColorWeight}>
